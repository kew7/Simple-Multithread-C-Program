/*
	Creates a new thread each time the letter 'a' is typed.
...
// getrandom returns a random number between min and max, which must be in integer range.
...
void KbdFunc(void) /* Dispatch and count threads. */
{
	...
	do
	{
		...
		{
			ThreadNr++;
			_beginthread( (_beginthread_proc_type) BounceProc, 0, &ThreadNr );
			WriteTitle(ThreadNr);
		}
...
{
	char NThreadMsg[80];
	sprintf(NThreadMsg, "Threads running: %02d. Press 'A' to start a thread,'Q' to quit.\n", ThreadNum);
	printf(NThreadMsg);					// printing to the console
	SetConsoleTitle(NThreadMsg);		// sets the current title for the current console
}
